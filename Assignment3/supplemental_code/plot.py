...
def plot_learned_matches(landmarks_true, landmarks_pred, img, img_name, n_iters):
    plt.figure()
    plt.imshow(img)
    plt.scatter(landmarks_true[0, :], landmarks_true[1, :], label='detected landmarks', marker='o', c='b')
    if landmarks_pred is not None:
        landmarks_preds_x, landmarks_pred_y = landmarks_pred.detach()[0, :], landmarks_pred.detach()[1, :]
        plt.scatter(landmarks_preds_x, landmarks_pred_y, label='predicted landmarks', marker='o', c='r')
    plt.tight_layout()
    plt.legend()
