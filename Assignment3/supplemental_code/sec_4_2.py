...
def texture(bfm, image='leonardo'):
    ...
    G_trans = V @ P @ T @ G
    # print(G_trans.shape)
    # G_trans = h2e(G_trans.T).T
    # print(G_trans.shape)
    G_trans = np.asarray(G_trans[:3, :] / G_trans[3, :])
    plt.figure()
    plt.imshow(img)
    ...
    interpolated = np.empty_like(G_trans)
    print(G_trans.shape)
    for p_i in np.arange(G_trans.shape[1]):
        x = G_trans[0, p_i]
        y = G_trans[1, p_i]
    ...
    rendered_img = render(G_trans.T, texture, triangle_topology.T, H=height, W=width)
