...
# BFM 3D face model
class BFM():
    def __init__(self,tri=None, model_path = 'BFM/BFM_model_front.mat'):
        ...
        n_base_tri = len(model['tri'])
        if tri is not None:
            point_buf = model['point_buf']

            point_buf = [[p1 for p1 in p if p1 > n_base_tri - 1] for p in point_buf]

            point_map = defaultdict(list)
            for i in range(n_base_tri, len(tri)):
                v0, v1, v2 = tri[i]
                point_buf[v0].append(i)
                point_buf[v1].append(i)
                point_buf[v2].append(i)

            point_buf_len = max([len(x) for x in point_buf])

            point_buf = [pp + [len(tri)] * (point_buf_len - len(pp)) for pp in point_buf]

            self.point_buf = tf.constant(point_buf)
        else:
...
class Face3D():
    def __init__(self, tri=None):
        facemodel = BFM(tri=tri)
        self.facemodel = facemodel

    # analytic 3D face reconstructions with coefficients from R-Net
    def Reconstruction_Block(self,coeff, texture_map, texture_image, batchsize):
        """ coeff is to compute uv coordinates, coeff2 is for everything else
        """
        ...
        projection = self.Projection_block(face_shape_t)
        img_size = 224
        projection = tf.stack([projection[:,:,0],img_size - 1. - projection[:,:,1]],axis = 2)
        self.uv = tf.constant([[[0, 1]]], dtype=tf.float32) + projection / tf.constant([[[img_size, -img_size]]], dtype=tf.float32)
        ...
        landmark_p = tf.stack([landmark_p[:,:,0],img_size - 1. - landmark_p[:,:,1]],axis = 2)
        ...
        if not is_windows:
            render_imgs, z_buffer, norm_imgs = self.Render_block(face_shape_t,norm_r,face_color,self.facemodel,
                                                                 texture_map, texture_image, batchsize)
    ...
    def Render_block(self,face_shape,face_norm,face_color,facemodel,texture_map,texture_image,batchsize):
        ...
        if not is_windows:
            ...
            with tf.device('/cpu:0'):
                img, z_buffer, norm_imgs = texturing.mesh_renderer_texture(face_shape,
                    ...
                    texture_map = self.uv if texture_map is None else texture_map,
                    texture_image=texture_image, # batch * Ht * Wt * 3
