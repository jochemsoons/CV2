...
# HELPER FILE CONTAINING CODE FOR PLOTTING
from plot import *
...
def icp(A1, A2, epsilon, sample_size, closest_point_search='KDTree', sampling_method='a', noise_sigma=None, sample_target=False, max_distance=None):
    """
    ...
    while True:
        ...
        if abs(rms_prev - rms_current) <= epsilon or rms_current - rms_prev > 0.05:
...
##################################################################################
# Code to reproduce section 2.1

"""Uncomment to load MAT source and target""""
A1 = scipy.io.loadmat('Data/source.mat')['source'].T
A2 = scipy.io.loadmat('Data/target.mat')['target'].T

"""Uncommment to load dragon source and target"""
# A1, A2, A1_normal, A2_normal = create_test_points('dragon')

# Loop over each ICP variant.
for search_method in ['Brute', 'KDTree']:
    # Loop over each sampling variant. NOTE: Dragon cannot be used with method a (computationally too expensive)
    for method in ['a', 'b', 'c', 'e']:
        print("Method: " + search_method)
        print("Sample method: " + method)
        # Compute the rotation/translation and the RMSE + time scores.
        R, t, RMS_list, time_list = icp(A1, A2, 
                                    epsilon=1e-4, 
                                    sample_size=200, 
                                    closest_point_search=search_method, 
                                    sampling_method=method, 
                                    noise_sigma=1, 
                                    sample_target=False, 
                                    max_distance=None)
        # Compute final transformation
        final_transform_A1 = np.dot(A1, R.T) + t

        # Dump files for printing
        pdump(RMS_list, search_method + '_' + method + '_rms')
        pdump(time_list, search_method + '_' + method + '_time')

        # Merge and show the frames.
        merge = np.concatenate((A2, final_transform_A1))
        show_point_cloud(merge, method=method)

# Create the RMSE versus iterations plot.
plot_iterations_rms('Brute', 'MAT', '10')
plot_iterations_rms('KDTree', 'MAT', '10')
...
def merge_poses(step_size, epsilon, sample_size,  closest_point_search, sampling_method, sample_target, max_distance, question):
    target, target_normal = load_data("Data/data/0000000000.pcd")
    prev_frame, prev_frame_normal = load_data("Data/data/0000000000.pcd")
    ...
    for i in range(start_frame, 100, step_size):
        print("#### Stitching frame {} ####".format(i))
        if i < 10:
            path = "Data/data/000000000{}.pcd".format(i)
        else:
            path = "Data/data/00000000{}.pcd".format(i)
        cur_frame, cur_frame_normal = load_data(path)
        if question == "3.1b":
            R, t, _ = icp(cur_frame, prev_frame, cur_frame_normal, prev_frame_normal, epsilon, sample_size, closest_point_search, sampling_method, sample_target, max_distance)
            prev_frame = np.copy(cur_frame)
            pref_frame_normal = np.copy(cur_frame_normal)
            R_list.append(R)
            t_list.append(t)
            for j in range(len(R_list)-1, -1, -1):
                temp = np.dot(cur_frame, R_list[j].T) + t_list[j]
                cur_frame = temp
        elif question == "3.2":
            R, t, _ = icp(cur_frame, prev_frame, cur_frame_normal, prev_frame_normal, epsilon, sample_size, closest_point_search, sampling_method, sample_target, max_distance)
            cur_frame = np.dot(cur_frame, R.T) + t
        temp = np.concatenate((target, cur_frame))
        target = temp

    show_point_cloud(target, name="3.1a")
    show_point_cloud(target)

def run_assignment(question):
    if question == "3.1a":
        #####  Estimate the camera poses using two consecutive frames of given data. #####
        merge_poses(1, epsilon=0.0000015, sample_size=1000, closest_point_search='KDTree', sampling_method='e', sample_target=False, max_distance=None, question=question)
    elif question == "3.1b":
        ##### Estimate the camera pose and merge the results using every 2nd, 4th, and 10th frames. #####
        step_size = int(input("Enter desired step_size taken between frames: "))
        merge_poses(step_size, epsilon=0.0000015, sample_size=1000, closest_point_search='KDTree', sampling_method='e', sample_target=False, max_distance=None, question=question)
    elif question == "3.2":
        ##### Iteratively merge and estimate the camera poses for the consecutive frames. #####
        merge_poses(1, epsilon=0.0000015, sample_size=1000, closest_point_search='KDTree', sampling_method='e', sample_target=True, max_distance=0.05, question=question)
